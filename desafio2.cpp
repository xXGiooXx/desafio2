...
int main()
{
    ...
    cout<<"Por favor ingrese la cantidad de Alumnos a registrar: ";
            ...
            float notas[m];
            float suma_notas, promedio_alumno, promedio_global, tempPromedio;
            float promedios_alumnos[n];
            system("cls");
            /*
...
do{
        ...
        switch(menu)
        {
                ...
                for(int i=0;i<n;i++)
                    {
                        for(int j=0;j<m;j++)
                        {
                            notas[j] = mi_arreglo[i][j];
                            suma_notas += notas[j];
                        }
                        promedio_alumno = suma_notas / m; 
                        suma_notas = 0;
                        promedio_global += promedio_alumno;
                    }
                    promedio_global = promedio_global / n;
                    cout<<"Promedio global: "<<fixed<<setprecision(2)<<promedio_global<<endl;
                    promedio_global = 0;
            ...
            case 'd':
                ...
                for(int i=0;i<n;i++)
                    {
                        ...
                        promedios_alumnos[i] = suma_notas / m;
						suma_notas = 0;
					}
				...
				for(int i=0; i<n; i++)
					{
						for(int j=0; j<n; j++)
						{
							if(promedios_alumnos[j]<promedios_alumnos[j+1])
							{
								// Intercambio de promedios
				                tempPromedio = promedios_alumnos[j];
				                promedios_alumnos[j] = promedios_alumnos[j + 1];
				                promedios_alumnos[j + 1] = tempPromedio;
				...
				cout << "Alumnos ordenados por promedio de mayor a menor:" << endl;
			    for (int i = 0; i < 3; i++) {
			        cout << "Lugar #"<< i+1 << " " << nombres[i] << " - " <<fixed<<setprecision(2)<<promedios_alumnos[i]<<endl;
			    }	
